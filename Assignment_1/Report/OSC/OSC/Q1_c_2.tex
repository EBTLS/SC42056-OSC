...
\fi
    ...
    \subsection{Question 1(c) B}
        ...
        $C_x(n)$ is the maintenance cost of year n for X air conditioners, $C_y(n)$ is the maintenance cost of year n for Y air conditioners.
        %可以写成空调X，air conditioner X        
        \subsubsection{Model}
        ...
        Different durations lead to different amounts of maintenance budget can be used as installation budget, so the analysis to simplify the calculation in Question 1 (c) can not be done anymore. We calculated the maximum value for different durable years separately and found the optimal one from them. 
